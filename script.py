...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, use_cache=False, is_generation=False,**kwargs):
        B, T, C = x.shape
        ...
        if not is_generation or self.k_cache is None:
            sin, cos = self.rope(T, x.device)
            sin = sin.unsqueeze(1)
            cos = cos.unsqueeze(1)
        else:
            pos_offset = 0 if self.k_cache is None else self.k_cache.size(2)
            sin, cos = self.rope(T + pos_offset, x.device)
            sin = sin.unsqueeze(1)[:, :, pos_offset:pos_offset+T]
            cos = cos.unsqueeze(1)[:, :, pos_offset:pos_offset+T]
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, use_cache=False,**kwargs):
        x = x + self.attn(self.norm1(x), use_cache=use_cache,**kwargs)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, tokens_seq, kv_cache=None, return_cache=False, is_generation=False,**kwargs):
        seq_len, batch_size = tokens_seq.shape
        ...
        for block in self.blocks:
            x = block(x, use_cache=self.use_cache, is_generation=(is_generation and self.use_cache))
...
# Need to Prefil #
def generate_text_KV(model, enc, init_text, max_new_tokens=500, device="cpu",
                  top_p=None,
    ...
    with torch.no_grad():
        ...
        for step_i in range(max_new_tokens):
            ...
            next_token_tensor = torch.tensor([[chosen_token]], dtype=torch.long, device=device)
            logits_seq = model(next_token_tensor, is_generation=True)
...
def main():
    ...
    for model_name, model in models.items():
        ...
        with torch.no_grad():
            # 1) Greedy
            text_greedy, ann_greedy = generate_text_KV(
                model, enc, args.prompt, max_new_tokens=100, device=device,
                top_p=None,
            )
            # 2) top-p=0.95
            text_topp, ann_topp = generate_text_KV(
                model, enc, args.prompt, max_new_tokens=100, device=device,
                top_p=0.95,
            )
            # 3) top-p=1.0 => full distribution random sampling
            text_topp1, ann_topp1 = generate_text_KV(
                model, enc, args.prompt, max_new_tokens=100, device=device,
                top_p=1.0,
            )
